  ...
  var ProductVariants = /*#__PURE__*/function () {
    ...
    _createClass(ProductVariants, [{
      ...
      value: function _onOptionChanged(event, target) {
        ...
        let colorOptionIndex = this.productData.options.indexOf('Color');
        if(colorOptionIndex >= 0) {
          var colorElement = document.querySelector(`input[value="${this.currentVariant.options[colorOptionIndex]}"][data-option-position="${colorOptionIndex+1}"]`)
          colorElement.checked = true
          var selectedValueElement = colorElement.closest('.product-form__option').querySelector('.product-form__selected-value');
          if (selectedValueElement) {
            selectedValueElement.innerHTML = colorElement.value;
          }
          this['option' + (colorOptionIndex + 1)] = this.currentVariant.options[colorOptionIndex];
        }
      ...
      value: function _getCurrentVariantFromOptions() {
        ...
        if(!found && colorOptionIndex >= 0) {
          found = this.productData['variants'].find((variant) => {
            return variant.options.reduce((result, option, index) => {
              return result && (index == colorOptionIndex || option == _this3['option' + (index+1)])
            }, true)
          })
        }
        return found || null;
